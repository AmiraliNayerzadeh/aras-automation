    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            @can('create', App\Models\User::class)
                <a href="{{ route('admin.users.create') }}" class="btn btn-dark btn-sm">{{ __('app.create_new') }}</a>
            @endcan
        </div>
    </x-slot>
                ...
                <tbody>
                    @forelse ($users as $user)
                        <tr>
                            ...
                            <td><span class="badge text-bg-light border">{{ $user->status?->value }}</span></td>
                            <td class="text-end">
                                @can('update', $user)
                                    <a href="{{ route('admin.users.edit', $user) }}" class="btn btn-outline-secondary btn-sm">{{ __('app.edit') }}</a>
                                @endcan
                                @can('delete', $user)
